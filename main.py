...
def parse_args():
    parser = argparse.ArgumentParser(description="Training a CNN on the iNaturalist dataset")
    ...
    parser.add_argument("--filter_sizes", type=int, nargs="+", default=[3, 3, 3, 3, 3], 
    ...
    parser.add_argument("--dense_activation", type=str, default="relu", 
                        choices=["relu", "gelu", "silu", "mish"], 
                        help="Activation function")
    parser.add_argument("--dense_neurons", type=int, default=128, help="Number of neurons in dense layer")
...
def sweep_train():
    ...
    with wandb.init(name="utk") as run:
        args = parse_args()
        config = wandb.config
        model_name = f"{config.get('filter_config')}_bs_{config.get('batch_size')}_dn{config.get('dense_neurons')}_f{config.get('base_filters')}_ks{config.get('filter_sizes')}_act{config.get('activation')}_ep{config.get('max_epochs')}_lr{config.get('learning_rate')}"
        run.name = model_name
        run.save()
...
def regular_train():
    ...
    if config.get('run_name') is None:
        try:
            ...
            config['run_name'] = f"{config.get('filter_config')}_bs{config.get('batch_size')}_dn{config.get('dense_neurons')}_f{config.get('base_filters')}_ks{filter_sizes_str}_act{config.get('activation')}_ep{config.get('max_epochs')}_lr{config.get('learning_rate')}"
        except:
            # Fallback if something goes wrong with the run name generation
            config['run_name'] = f"cnn_training_run"
